...
This will write a `data.csv` file with the reduced data.

Next, the faces are detected:
...
Which will place a `data_faces_train.csv` as output.

To generate the file used by the `train.py` script, run the script:
...
This will generate a file `data_faces_train.csv`, which contains variations of the original images and landmarks.
